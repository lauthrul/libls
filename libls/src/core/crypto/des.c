...
void IP(uint32_t state[], const uint8_t in[])
{
...
void des_key_setup(const uint8_t key[DES_BLOCK_SIZE], uint8_t schedule[][6], DES_MODE mode)
{
    uint32_t i, j, to_gen, C, D;
    const uint32_t key_rnd_shift[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
...
void des_crypt(const uint8_t in[DES_BLOCK_SIZE], uint8_t out[DES_BLOCK_SIZE], const uint8_t key[][6])
{
...
int32_t des_encrypt_cbc(const uint8_t in[],         // Plaintext
...
{
    uint8_t buf_in[DES_BLOCK_SIZE], buf_out[DES_BLOCK_SIZE], iv_buf[DES_BLOCK_SIZE];
    int blocks, idx;

    if (in_len % DES_BLOCK_SIZE != 0)
        return (FALSE);

    blocks = in_len / DES_BLOCK_SIZE;

    memcpy(iv_buf, iv, DES_BLOCK_SIZE);

    for (idx = 0; idx < blocks; idx++)
    {
        memcpy(buf_in, &in[idx * DES_BLOCK_SIZE], DES_BLOCK_SIZE);
        xor_buf(iv_buf, buf_in, DES_BLOCK_SIZE);
        des_crypt(buf_in, buf_out, key);
        memcpy(&out[idx * DES_BLOCK_SIZE], buf_out, DES_BLOCK_SIZE);
        memcpy(iv_buf, buf_out, DES_BLOCK_SIZE);
    }
                        ...
                        const uint8_t iv[DES_BLOCK_SIZE])        // IV, must be DES_BLOCK_SIZE bytes long
{
    ...
    for (idx = 0; idx < blocks; idx++)
    {
        memcpy(buf_in, &in[idx * DES_BLOCK_SIZE], DES_BLOCK_SIZE);
        des_crypt(buf_in, buf_out, key);
        xor_buf(iv_buf, buf_out, DES_BLOCK_SIZE);
        memcpy(&out[idx * DES_BLOCK_SIZE], buf_out, DES_BLOCK_SIZE);
        memcpy(iv_buf, buf_in, DES_BLOCK_SIZE);
    }
...
void three_des_crypt(const uint8_t in[DES_BLOCK_SIZE], uint8_t out[DES_BLOCK_SIZE], const uint8_t key[][16][6])
{
                              ...
                              const uint8_t iv[DES_BLOCK_SIZE])   // IV, must be DES_BLOCK_SIZE bytes long
{
    ...
    for (idx = 0; idx < blocks; idx++)
    {
        memcpy(buf_in, &in[idx * DES_BLOCK_SIZE], DES_BLOCK_SIZE);
        xor_buf(iv_buf, buf_in, DES_BLOCK_SIZE);
        three_des_crypt(buf_in, buf_out, key);
        memcpy(&out[idx * DES_BLOCK_SIZE], buf_out, DES_BLOCK_SIZE);
        memcpy(iv_buf, buf_out, DES_BLOCK_SIZE);
    }
