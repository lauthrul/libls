...
* AES - CBC
*******************/
int aes_encrypt_cbc(const uint8_t in[], size_t in_len, uint8_t out[], const uint32_t key[], int keysize, const uint8_t iv[AES_BLOCK_SIZE])
{
...
int aes_encrypt_cbc_mac(const uint8_t in[], size_t in_len, uint8_t out[], const uint32_t key[], int keysize, const uint8_t iv[AES_BLOCK_SIZE])
{
	uint8_t buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
int aes_encrypt_ccm(const uint8_t payload[], uint32_t payload_len, const uint8_t assoc[], unsigned short assoc_len,
                    ...
                    uint32_t mac_len, const uint8_t key_str[], int keysize)
{
	...
	increment_iv(temp_iv, AES_BLOCK_SIZE - 1 - mac_len);   // Last argument is the uint8_t size of the counting portion of the counter block. /*BUG?*/
...
int aes_decrypt_ccm(const uint8_t ciphertext[], uint32_t ciphertext_len, const uint8_t assoc[], unsigned short assoc_len,
                    const uint8_t nonce[], unsigned short nonce_len, uint8_t plaintext[], uint32_t *plaintext_len,
                    uint32_t mac_len, int *mac_auth, const uint8_t key_str[], int keysize)
{
	...
	*plaintext_len = ciphertext_len - mac_len;
	...
	memcpy(temp_iv, counter, AES_BLOCK_SIZE);
	increment_iv(temp_iv, AES_BLOCK_SIZE - 1 - mac_len);   // (AES_BLOCK_SIZE - 1 - mac_len) is the uint8_t size of the counting portion of the counter block.
...
// Creates the first counter block. First uint8_t is flags, then the nonce, then the incremented part.
void ccm_prepare_first_ctr_blk(uint8_t counter[], const uint8_t nonce[], int nonce_len, int payload_len_store_size)
{
...
void ccm_prepare_first_format_blk(uint8_t buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const uint8_t nonce[], int nonce_len)
{
	// Set the flags for the first uint8_t of the first block.
...
// Substitutes a uint32_t using the AES S-Box.
uint32_t SubWord(uint32_t uint32_t)
{
	unsigned int result;

	result = (int)aes_sbox[(uint32_t >> 4) & 0x0000000F][uint32_t & 0x0000000F];
	result += (int)aes_sbox[(uint32_t >> 12) & 0x0000000F][(uint32_t >> 8) & 0x0000000F] << 8;
	result += (int)aes_sbox[(uint32_t >> 20) & 0x0000000F][(uint32_t >> 16) & 0x0000000F] << 16;
	result += (int)aes_sbox[(uint32_t >> 28) & 0x0000000F][(uint32_t >> 24) & 0x0000000F] << 24;
	return(result);
}
...
// Performs the AddRoundKey step. Each round has its own pre-generated 16-uint8_t key in the
...
void aes_encrypt(const uint8_t in[AES_BLOCK_SIZE], uint8_t out[AES_BLOCK_SIZE], const uint32_t key[], int keysize)
{
	uint8_t state[4][4];
...
void aes_decrypt(const uint8_t in[AES_BLOCK_SIZE], uint8_t out[AES_BLOCK_SIZE], const uint32_t key[], int keysize)
{
	uint8_t state[4][4];
...
void print_rnd_key(uint32_t key[])
{
	int idx;
