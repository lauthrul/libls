...
void aes_key_setup(const uint8_t key[],             // The key, must be 128, 192, or 256 bits
                   ...
                   int32_t keysize);                // Bit length of the key, 128, 192, or 256

void aes_encrypt(const uint8_t in[AES_BLOCK_SIZE],  // 16 bytes of plaintext
                 uint8_t out[AES_BLOCK_SIZE],       // 16 bytes of ciphertext
                 const uint32_t key[],              // From the key setup
                 int32_t keysize);                  // Bit length of the key, 128, 192, or 256

void aes_decrypt(const uint8_t in[AES_BLOCK_SIZE],  // 16 bytes of ciphertext
                 uint8_t out[AES_BLOCK_SIZE],       // 16 bytes of plaintext
                 const uint32_t key[],              // From the key setup
                 int32_t keysize);                  // Bit length of the key, 128, 192, or 256
...
// AES - CBC
///////////////////
int32_t aes_encrypt_cbc(const uint8_t in[],         // Plaintext
                        size_t in_len,              // Must be a multiple of AES_BLOCK_SIZE
                        uint8_t out[],              // Ciphertext, same length as plaintext
                        const uint32_t key[],       // From the key setup
                        int32_t keysize,            // Bit length of the key, 128, 192, or 256
                        const uint8_t iv[AES_BLOCK_SIZE]);      // IV, must be AES_BLOCK_SIZE bytes long

// Only output the CBC-MAC of the input.
int32_t aes_encrypt_cbc_mac(const uint8_t in[],     // plaintext
                            size_t in_len,          // Must be a multiple of AES_BLOCK_SIZE
...
int32_t aes_decrypt_cbc(const uint8_t in[],         // Ciphertext, must be a multiple of AES_BLOCK_SIZE
                        size_t in_len,              // Must be a multiple of AES_BLOCK_SIZE
...
void increment_iv(uint8_t iv[],                     // Must be a multiple of AES_BLOCK_SIZE
                  int32_t counter_size);            // Bytes of the IV used for counting (low end)

void aes_encrypt_ctr(const uint8_t in[],            // Plaintext
                     size_t in_len,                 // Any uint8_t length
                     uint8_t out[],                 // Ciphertext, same length as plaintext
                     const uint32_t key[],          // From the key setup
                     int32_t keysize,               // Bit length of the key, 128, 192, or 256
...
int32_t aes_encrypt_ccm(const uint8_t plaintext[],              // IN  - Plaintext.
                        ...
                        int32_t keysize);                       // IN  - The length of the key in bits. Valid values are 128, 192, 256.
...
int32_t aes_decrypt_ccm(const uint8_t ciphertext[],             // IN  - Ciphertext, the concatination of encrypted plaintext and MAC.
                        ...
                        int32_t* mac_auth,                      // OUT - TRUE if authentication succeeded, FALSE if it did not. NULL pointer will ignore the authentication.
