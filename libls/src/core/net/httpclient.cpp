...
namespace lslib
{
    ...
    namespace net
    {
        ...
        SHttpResult CHttpClient::HttpGet(const SHttpGetParam& vParam)
        {
            ...
            {
                // perform
                Perform(strSid, pCurl, vParam, vResult);
...
label_exit:
        ...
        int CHttpClient::Perform(const string& sid, CURL* pCurl, const SHttpParam& vParam, __inout__ SHttpResult& vResult)
        {
            if (!IsInit()) Init();
        ...
        string CHttpClient::DumpParamText(SHttpParam* pParam)
        {
            ...
            switch (pParam->eMethod)
            {
                ...
                case HTTP_DOWNLOAD:
                    ...
                    strText += strtool::format("callback: %p, ", ((SHttpDowloadParam*)pParam)->cb);
                    strText += strtool::format("clientp: %p", ((SHttpDowloadParam*)pParam)->clientp);
                    break;
                case HTTP_UPLOAD:
                    strText += strtool::format("file: %s, ", ((SHttpDowloadParam*)pParam)->strFile.c_str());
