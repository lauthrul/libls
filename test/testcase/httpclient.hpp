...
void url_crack_test()
{
    ...
    {
        const string& url = sz[i];
        vUrl = CrackUrl(url.c_str());
        cout << "[" << url << "]" << endl;
        ...
        cout << "query: " << endl;
        for (map<string, string>::iterator it = vUrl.mapQuerys.begin();
...
void http_get_test()
{
    ...
    SHttpGetParam v;
    SHttpResult r;
    for (size_t i = 0; i < sizeof(sz1) / sizeof(string); i++)
    {
        v.strUrl = sz1[i];
...
void http_post_test()
{
    ...
    SHttpPostParam vp;
    SHttpResult r;
    for (size_t i = 0; i < sizeof(sz2) / sizeof(string) / 2; i++)
    {
        vp.strUrl = sz2[i];
        vp.strPost = sz2[i + 1];
...
void http_download_test()
{
    ...
    SHttpResult r;
    for (size_t i = 0; i < sizeof(sz3) / sizeof(string); i++)
    {
        vd.strUrl = sz3[i];
